...
export class MeasureController {
  ...
  @ApiBody({
    description: 'Request body for measure upload',
  ...
  @ApiResponse({
    status: 404,
    description: 'Já existe uma leitura para este tipo no mê atual',
  })
  @Post('upload')
  async upload(@Body() data: MeasureUploadRequestDTO): Promise<any> {
    return this.measureService.upload(data);
  }

  @ApiBody({
    description: '0 - desconfirmar / 1 - confirmar',
  ...
  @Patch('confirm')
  async confirm(@Body() data: MeasureUpdateDTO): Promise<any> {
  ...
  async findMeasures(
    ...
    if (measure_type) {
      ...
      if (!enumMeasureType) {
        throw new InvalidTypeIncorrectException(
